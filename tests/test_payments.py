...
async def test_create_order():
    ...
    # 1. Photos
    mock_photo = MagicMock(id=1, album_id=10)
    ...
    # 2. Albums
    mock_album = MagicMock(id=10, event_id=100)
    mock_album_result = MagicMock()
    mock_album_scalars = MagicMock()
    mock_album_scalars.all.return_value = [mock_album]
    mock_album_result.scalars.return_value = mock_album_scalars
    
    # 3. Events
    mock_event = MagicMock(id=100, user_id=99, price_per_photo=1000)
    mock_event_result = MagicMock()
    mock_event_scalars = MagicMock()
    mock_event_scalars.all.return_value = [mock_event]
    mock_event_result.scalars.return_value = mock_event_scalars
    
    mock_db.execute.side_effect = [
        mock_photo_result, # fetch photos
        mock_album_result, # fetch albums
        mock_event_result  # fetch events
    ]
