...
async def test_archive_callback_not_found(async_client):
    ...
    response = await async_client.post(
        "/api/v1/orders/1/archives/999/callback", 
...
async def test_archive_callback_success(async_client, db_session):
    ...
    from app.core.config import settings
    response = await async_client.post(
        f"/api/v1/orders/1/archives/{archive.id}/callback",
        json={"status": ArchiveStatus.COMPLETED.value, "s3_object_key": "archives/test.zip", "size": 1024},
        headers={"x-archive-secret": settings.SECRET_KEY}
    )
    assert response.status_code == 200
